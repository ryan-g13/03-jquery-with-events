...
articleView.handleMainNav = function() {
  ...
  $('.main-nav .tab').on('click', function(){
    let $navClicked = $(this).data('content');
    if ($(this).data('content') === 'about') {
      $('.tab-content').hide();
      $(`#${$navClicked}`).fadeIn(750);
    } else if ($(this).data('content') === 'articles') {
      $(`#${$navClicked}`).fadeIn(750);
      $('#about').hide();
      $('.template').hide();
    }
...
articleView.setTeasers = function() {
  ...
  $('article').delegate('a','click', function() {
    let $revealed = $(this);
    $revealed.hide();
    $('.article-body *:nth-of-type(n+2)').show(); //traverse element upwards from the a tag to the article-body to display
